...
![Experimental settings and evaluation framework](plots/Figure_2_Neurips.png)
...
* `honest_rollout.py`: Honest autoregressive rollout baseline (no teacher forcing).
* `manual_theta_fit.py`: Manual / analytical theta fitting baseline.
* `neural_ode.py`: Pure neural ODE baseline (no mechanistic structure).
* `ode_rnn.py`: Original ODE-RNN (Rubanova et al. 2019) baseline.
* `ode_rnn_analytical.py`: ODE-RNN with an analytical ODE layer.

## Contributing

>📋  *Pick a licence (e.g., MIT, GPL) and describe how to contribute to your code repository. Explain PR guidelines, code formatting rules, etc.*